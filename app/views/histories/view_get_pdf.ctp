<?php
	...
	$tcpdf->xheadertext = 'Files Prelevati';
	$tcpdf->xfootertext = 'Files Prelevati - Page ';
	...
	$tcpdf->Cell(0,10, "Files Prelevati", 0,1,'L',1);
	...
	// set header info
	$w = array(10, 20, 30, 20, 20, 68, 22);
	$header = array('Num', 'Username', 'Realname', 'Type', 'AgeGroup', 'Filename', 'Date');
	...
	foreach($report_pdf as $report)
	{
		...
		$tcpdf->Cell($w[5], 6, $report['ActivityFilegetHistory']['activity_fileget_history_absfilename'], 0, 0, 'LR', $fill,'',1);
		$tcpdf->Cell($w[6], 6, date("d-m-Y H:i",$report['ActivityFilegetHistory']['activity_fileget_history_date']), 0, 0, 'LR', $fill);

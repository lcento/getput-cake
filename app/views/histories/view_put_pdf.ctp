<?php
	...
	$tcpdf->xheadertext = 'Files Inviati';
	$tcpdf->xfootertext = 'Files Inviati - Page ';
	...
	$tcpdf->Cell(0,10, "Files Inviati", 0,1,'L',1);
	...
	// set header info
	$w = array(10, 20, 30, 20, 20, 53, 15, 22);
	$header = array('Num', 'Username', 'Realname', 'Type', 'AgeGroup', 'Filename', 'Dest', 'Date');
	...
	foreach($report_pdf as $report)
	{
		...
		$tcpdf->Cell($w[5], 6, $report['ActivityFileputHistory']['activity_fileput_history_absfilename'], 0, 0, 'LR', $fill,'',1);
		$tcpdf->Cell($w[6], 6, $report['ActivityFileputHistory']['activity_fileput_history_filedest'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[7], 6, date("d-m-Y H:i",$report['ActivityFileputHistory']['activity_fileput_history_date']), 0, 0, 'LR', $fill);

<?php
	...
	$tcpdf->xheadertext = 'Lista Utenti';
	$tcpdf->xfootertext = 'Lista Utenti - Page ';
	...
	$tcpdf->Cell(0,10, "Lista Utenti", 0,1,'L',1);
	...
	// set header info
	$w = array(10, 20, 20, 30, 30, 20, 30, 30);
	$header = array('Num', 'Username', 'Password', 'Realname', 'Level', 'Type', 'AgeGroup', 'Author');
	...
	foreach($report_pdf as $report)
	{
		...
		$tcpdf->Cell($w[2], 6, $report['User']['users_password'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[3], 6, $report['User']['users_username_realname'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[4], 6, $report['LevelUserDesc']['level_users_desc'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[5], 6, $report['TypeUserDesc']['type_users_desc'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[6], 6, $report['AgeGroup']['group_desc'], 0, 0, 'LR', $fill);
		$tcpdf->Cell($w[7], 6, $report['User']['users_se_author'], 0, 0, 'LR', $fill);
